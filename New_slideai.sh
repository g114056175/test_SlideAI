...
# SlideAI guided launcher (Linux x86_64; Ubuntu/Debian can install Docker automatically).
#
# This file intentionally leaves slideai.sh untouched.  Stable start/stop
# operations are delegated to that launcher, while first-run setup is handled
# here as a sequence of explicit, skippable steps.

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LEGACY_LAUNCHER="${PROJECT_ROOT}/slideai.sh"
...
run_legacy() {
  [[ -x "${LEGACY_LAUNCHER}" ]] || die "找不到可執行的 slideai.sh。"
  "${LEGACY_LAUNCHER}" "$@"
}
...
download_model_source() {
  ...
  if [[ "${source}" == hf:* ]]; then
    ...
    warn "New_slideai.sh 自動安裝目前支援 hf:repo 或既有本機資料夾。"
...
EOF
    read -r -p "請選擇 [0-6]：" choice
    case "${choice}" in
      1) run_legacy start; pause_screen ;;
      2) run_legacy stop; pause_screen ;;
      3) run_legacy restart; pause_screen ;;
...
if [[ -z "${command_name}" ]]; then
  ...
  case "${command_name}" in
    start) run_legacy start ;;
    stop) run_legacy stop ;;
    restart) run_legacy restart ;;
    status) status_report ;;
    settings|config) settings_menu ;;
    build|setup) build_wizard ;;
    -h|--help|help)
      say "用法：./New_slideai.sh [start|stop|restart|status|settings|build]"
      ;;
